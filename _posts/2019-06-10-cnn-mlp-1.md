...
### Derivation
#### Convolution
Assume $$M \in \mathbb{R}^{n \times m}$$ is the input signal, $$K \in \mathbb{R}^{(2 \cdot k + 1)\times(2 \cdot k + 1)}$$ is the convolution kernel (the "filter") with $$n,m,k \in \mathbb{N}$$
...
We define a matrix $$W_{\upsilon \chi} \in \mathbb{R}^{n \times m}$$ as
...
and an operation $$\text{dot}: \mathbb{R}^{n \times m} \times \mathbb{R}^{n \times m} \to \mathbb{R}$$ as

$$\text{dot}(A,B) = \sum_{i=0}^n\sum_{j=0}^m A(i,j) \cdot B(i,j)$$

which is a kind of generalized dot product.

#### Convolution and the generalized dot product

Using this product we can see:
...
This shows, that the convolution is the same as our generalized dot product if the second operator is choosen as shown above.

#### Simplifying the generalized dot product to the normal dot product
To illustrate the conversion we defined us the generalized dot product, this allowed us to keep our input data in 2D-Form. However most MLPs (for example the OpenCV-Implementation which inspired this post) work only on 1D-Data that is vectors. So we need to convert our data into vectors, for this we define the operation
$$\text{reshape}: \mathbb{R}^{n \times m} \to \mathbb{R}^{n \cdot m}$$ as

$$\text{reshape}(M)(i) = x(i) = M(\text{floor}(i / n), i \mod n)$$
...
Now we can show that reshaping and applying the normal dot product is equivalent to the generalized dot product:

$$\text{dot}(A,B) = \sum_{i=0}^n\sum_{j=0}^m A(i,j) \cdot B(i,j)$$

$$= \sum_{i=0}^{n \cdot m} A(\text{floor}(i / n), i \mod n) \cdot B(\text{floor}(i / n), i \mod n)$$

$$= \sum_{i=0}^{n \cdot m} \text{reshape}(A)(i) \cdot \text{reshape}(B)(i)$$

$$= \text{reshape}(A)^\text{T} \cdot \text{reshape}(B)$$

Using this operation we can now see that the generalized dot product which was equivalent to the convolution can now be written as a normal dot product:

$$(M \ast K)(\upsilon, \chi) = \text{dot}(M, W_{\upsilon \chi}) = \text{reshape}(M)^\text{T} \cdot \text{reshape}(W_{\upsilon \chi})$$
...
For now we will ignore the bias and the activation function, see the next section for more information on these topics. 
...
the input of our layer $$a \in \mathbb{R}^{n \cdot m}$$ is the reshaped image as seen above:
...
\end{pmatrix} =
\begin{pmatrix}
    w_1^\text{T} \cdot \text{reshape}(M) \\
    w_2^\text{T} \cdot \text{reshape}(M) \\
    \vdots \\
    w_n^\text{T} \cdot \text{reshape}(M) \\
\end{pmatrix}$$
...
$$w_i = \text{reshape}(W_{\upsilon \chi})$$

with $$\upsilon = \text{floor}(i/n)$$ and $$\chi = i \mod n$$.

For the following layer our input data is already flattened, as a result we do not need to change anything inside the network during inference.
...
(see my last blog post for more information on transfer functions in OpenCV).
...
#### Pooling
Pooling is used to reduce the size of the feature map, for this multiple entrys of the feature map (usually in $$2 \times 2$$ grid) are combined to form a single
value.
 * **Max pooling:** The most commonly used pooling operation is max pooling which simply takes the maximum of all input values. Due to the non-space invariant properties of this operation cann't be implemented using an MLP.
 * **Average pooling:** Average pooling can be implemented using an MLP. The trivial approach is to add another layer after a convolution layer with a weight matrix
    ...
    this method has one major drawback: the activation function is now calculated twice instead of once. By using the taylor series of the $$\tanh$$ we can prove,
    that

    $$ x \approx \tanh(x)$$

    for small $$x$$. This means we only need to compress the inputs to our activation function and expand them later on. This compression can be achieved by 
    scaling $$W$$:
    ...
    This operation can not be combined with the calculation of the feature map, the reason for this is, that ($$f$$ is the nonlinear activation function):

    $$\frac{f(x) + f(y)}{2} \neq f\left(\frac{x + y}{2}\right)$$

 * **Stride:** By the definition a stride $$>1$$ is not pooling. A stride is simply used to only calculate certain points of the feature map, for the calculation there are two possible approaches, either by adding another layer, as seen above. This still requires scaling the feature map and assuming the $$\tanh$$ as linear for small $$x$$. The second approach is to calculate the second weight matrix and calculate a combined weight matrix:
